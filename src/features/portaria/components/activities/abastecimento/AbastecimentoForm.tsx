...
import { abastecimentoService } from '../../../services/activities/abastecimento';
...
export const AbastecimentoForm: React.FC<Props> = ({
  ...
  useEffect(() => {
    if (form.transportadora && form.transportadora !== 'outros') {
      abastecimentoService.getPlacasByTransportadora(form.transportadora)
  ...
  useEffect(() => {
    if (open && form.transportadora === 'outros') {
      const abastecimento = form.abastecimento;
      if (abastecimento?.placa_manual) {
        setPlacaOptions([{ id: 'manual', placa: abastecimento.placa_manual }]);
      }
    }
  }, [open, form.transportadora]);

  useEffect(() => {
    if (form.data && !form.dataSaida) {
      setForm({ ...form, dataSaida: form.data });
    }
  }, [form.data, form.dataSaida]);
