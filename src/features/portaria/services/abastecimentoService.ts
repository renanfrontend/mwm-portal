/**
 * PORTARIA ABASTECIMENTO - Service de Combos
 * Responsável por carregar transportadoras, tipos de veículo e placas para o
 * formulário de Abastecimento de forma isolada da Entrega de Dejetos.
 *
...
export interface BioVeiculoTransportadoraAbastecimento {
  id: number;
  bioTransportadora: {
    id: number;
    nomeFantasia: string;
...
export const abastecimentoService = {
   ...
   */
  async getBioVeiculosTransportadoras(): Promise<BioVeiculoTransportadoraAbastecimento[]> {
    console.log('📡 [ABASTECIMENTO] Chamando API: /bio-veiculos-transportadoras');
    const response = await api.get<BioVeiculoTransportadoraAbastecimento[]>('/bio-veiculos-transportadoras');
